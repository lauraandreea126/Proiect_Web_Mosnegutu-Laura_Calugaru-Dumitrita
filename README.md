...
• Laura (S2): Dezvoltarea primelor două Vizualizări NATIVE (ex: Bar Chart și Pie Chart). Graficele vor fi construite prin manipularea DOM-ului și generarea de etichete , , , din JavaScript, fără nicio bibliotecă externă.
...
• Laura (S2): Implementarea celei de-a 3-a vizualizări native SVG. Dezvoltarea logicii de descărcare a vizualizărilor în formatele SVG (descărcare directă a sursei DOM) și WebP (desenarea SVG-ului pe un ascuns folosind API-ul nativ).
...
• Dumitrița & Laura: Înregistrarea și editarea Filmulului Demonstrativ cu durata între 3 și 5 minute la calitate UHD. Pregătirea repository-ului final (fișier README detaliat și menționarea Licenței Libere aplicate codului).
 ...
 Obiectivul ProiectuluiScopul principal: Crearea unei aplicații web pentru vizualizarea nominalizărilor la premiile actorilor (SAG Awards) din ultimii 5 ani.  Funcționalități cheie: Căutarea și filtrarea actorilor, afișarea biografiilor și pozelor (via TMDb), un agregator de știri externe și generarea de statistici vizuale.  🛠️ Constrângeri Tehnice Stricte (Fără Framework-uri)Front-end: Se folosește exclusiv HTML5, CSS3 pur și Vanilla JavaScript (fără React, jQuery sau Bootstrap).  Back-end: Implementare în PHP nativ, fără framework-uri precum Laravel.  Bază de date: Utilizarea SQLite pentru stocarea datelor interne.  
